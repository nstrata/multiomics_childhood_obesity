...
here::i_am("script/c.Molecular drivers/shap_pathway_analysis_B_AC.R")
setwd(here::here())

# Load data 
top_shap_B_AC<-read.csv("./results/csvs/top_shap_B_AC.csv")

# First identify the top features  per layer and obtain vectors of features
shap_top<-top_shap_B_AC %>% t() %>% as.data.frame() 
...
write.csv(tc_genes_to_merge, "./results/csvs/tc_genes_B_AC.csv")
